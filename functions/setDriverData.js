...
if (userIDCookie) {
  ...
  if (userID) {
    const endpoint = `/API/getSingleDriverById.php?id=${userID}`;
    ...
    fetch(endpoint, {
      ...
      .then((data) => {
        ...
        localStorage.setItem("bcdash-driverData", JSON.stringify(data));
      })
      .catch((error) => {
  ...
  console.error("No bcdash_user cookie found");
}
